 ...
 * @tutorial
 */
var Jugador = function(nom_, tipus_, mode_joc_) {
    this.nom = nom_;
...
Jugador.prototype.afegir_carta_a_jugada_actual = function(carta_) {
    if (this.jugades.length == 0) {
        this.jugades[0] = new jugada(carta_, this);
    } else {
      // console.log("holaaa");
        this.jugades[this.jugada_actual].afegir_carta(carta_);
        console.log(  this.jugades[this.jugada_actual].puntuacio);

    }
    if (this.partida != null) {
        this.partida.getSetimigEngine().pintarPuntuacio(this);
    }
}
...
Jugador.prototype.estaJugant = function() {
    var jugant_ = false;
    for (var i = 0; i < this.jugades.length; i++) {
        var tancada_ = this.jugades[i].tancada
...
Jugador.prototype.afegirJugada = function(carta_) {
    ...
    if (carta_.valor <= 1 && this.jugades.length >= 1 && this.jugades.length < 4 && this.jugades[0].getNumCartes() == 1 && this.jugades[this.jugades.length - 1].getNumCartes() == 1 && (this.jugades[this.jugades.length - 1].getCarta(0).valor == 0.5 || this.jugades[this.jugades.length - 1].getCarta(0).valor == 1)) {
...
Jugador.prototype.esPosibleObrir = function(carta_) {
    ...
    if (carta_.valor <= 1 && this.jugades.length >= 1 && this.jugades.length < 4 && this.jugades[0].getNumCartes() == 1 && this.jugades[this.jugades.length - 1].getNumCartes() == 1 && this.jugades[this.jugades.length - 1].getCarta(0).valor <= 1) {
        return true;
        //Si no es compleixen les condicions anteriors el que fem es afegir la carta a la jugada actual
    } else {
        return false;
